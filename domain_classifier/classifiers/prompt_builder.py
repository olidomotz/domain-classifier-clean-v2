"""Improved prompt builder for better JSON formatting and confidence scores."""
...
def build_decision_tree_prompt(examples: Dict[str, List[Dict[str, str]]]) -> str:
    """
    Build an improved prompt that implements the decision tree approach with clearer JSON instructions.
...
Here are examples of correctly classified companies:"""
    ...
    # Add improved instructions for output format with clearer JSON guidance
...
2. For technology service businesses, provide DIFFERENT confidence scores for each category.
   - Managed Service Provider scores should range from 50-95 for clear MSPs
   - Commercial A/V scores should range from 60-95 for clear Commercial A/V businesses
   - Residential A/V scores should range from 60-95 for clear Residential A/V businesses
   - Use the upper range (80-95) when evidence is very strong and clear
...
YOUR RESPONSE MUST BE A SINGLE VALID JSON OBJECT WITH NO OTHER TEXT BEFORE OR AFTER. DO NOT USE DOUBLE QUOTES INSIDE QUOTED STRINGS - USE SINGLE QUOTES INSTEAD FOR NESTED QUOTES."""
